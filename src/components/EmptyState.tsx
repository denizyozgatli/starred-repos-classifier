...
import { SearchX, Inbox, AlertTriangle, RefreshCw, X, Upload } from 'lucide-react';

interface EmptyStateProps {
  type: 'search' | 'empty' | 'error' | 'initial';
  query?: string;
...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  if (type === 'initial') {
    return (
      <div className="flex flex-col items-center justify-center p-8 sm:p-12 text-center bg-[#161b22] border border-[#30363d] rounded-xl my-6">
        <div className="w-12 h-12 rounded-full bg-[#21262d] border border-[#30363d] flex items-center justify-center text-[#58a6ff] mb-3.5 shadow-inner">
          <Upload className="w-6 h-6" aria-hidden="true" />
        </div>
        <h3 className="text-base font-semibold text-white mb-1.5">No dataset loaded</h3>
        <p className="text-xs text-github-muted max-w-md mb-4 leading-relaxed">
          Import a <code className="text-[#58a6ff] bg-[#21262d] px-1.5 py-0.5 rounded text-[11px]">repos.json</code> file generated by Starred Repos Classifier CLI.
        </p>
        {onOpenImport && (
          <button
            type="button"
            onClick={onOpenImport}
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-[#238636] hover:bg-[#2ea043] text-white rounded-lg text-xs font-semibold focus-visible:ring-2 focus-visible:ring-[#58a6ff] focus-visible:outline-none transition-colors cursor-pointer shadow-sm"
          >
  ...
  if (type === 'error') {
    return (
      <div className="flex flex-col items-center justify-center p-8 sm:p-10 text-center bg-[#161b22] border border-[#f85149]/30 rounded-xl my-6">
        ...
        <p className="text-xs text-github-muted max-w-md mb-4 leading-relaxed">
          {message || 'An error occurred while loading the repository dataset.'}
        </p>
        {onReset && (
          <button
            type="button"
    ...
    return (
      <div className="flex flex-col items-center justify-center p-8 sm:p-10 text-center bg-[#161b22] border border-[#30363d] rounded-xl my-6">
        ...
        <p className="text-xs text-github-muted max-w-md leading-relaxed">
          No repositories found in the loaded dataset.
        </p>
      </div>
    );
  }
